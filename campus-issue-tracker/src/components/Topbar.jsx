...
// // }
// import React, { useState } from "react";
// import { Search, Bell, User, Sun, Moon, LogOut, Settings, Menu } from "lucide-react";
// import { motion, AnimatePresence } from "framer-motion";

// export default function Topbar({ onSearch, onToggleTheme, isDark, onHamburgerClick }) {
//   const [showProfile, setShowProfile] = useState(false);
//   const [showNotifs, setShowNotifs] = useState(false);

//   return (
//     <header className="sticky top-0 z-30 flex items-center justify-between gap-4 px-4 md:px-8 py-4 
//       bg-white/80 backdrop-blur-xl border-b border-gray-100 
...
//         className="md:hidden p-2 rounded-lg bg-gray-100 dark:bg-slate-900 shadow text-gray-700 dark:text-gray-300"
//       >
//         <Menu size={22} />
//       </button>
...
export default function Topbar({ onSearch, onToggleTheme, isDark }) {
  ...
  return (
    <header className="sticky top-0 z-30 flex items-center justify-between gap-4 px-4 md:px-8 py-4 
      ...
      {/* LOGO AREA (Visible on Mobile) */}
      <div className="md:hidden font-bold text-lg text-indigo-600 dark:text-indigo-400">
        CampusTracker
      </div>

      {/* SEARCH BAR (Hidden on Mobile for cleaner look, or make it an icon toggle) */}
      ...
      {/* RIGHT ACTIONS */}
      <div className="flex items-center gap-2 md:gap-3 relative ml-auto">
        
        {/* Mobile Search Icon (Optional: you can wire this up to show input) */}
           ...
           <AnimatePresence>
            {showNotifs && (
              <motion.div
                ...
                animate={{ opacity: 1, y: 0, scale: 1 }}
                exit={{ opacity: 0, y: 10, scale: 0.95 }}
                className="absolute right-0 mt-2 w-72 md:w-80 bg-white rounded-2xl shadow-xl border border-gray-100 p-4 z-50 origin-top-right
                dark:bg-slate-900 dark:border-slate-800"
              >
                <h3 className="font-semibold mb-3 dark:text-white">Notifications</h3>
                <div className="space-y-3">
        ...
        <div className="relative">
          ...
          >
           ...
           {/* Profile Menu (Keep existing code) */}
           <AnimatePresence>
            {showProfile && (
              <motion.div
                initial={{ opacity: 0, y: 10, scale: 0.95 }}
